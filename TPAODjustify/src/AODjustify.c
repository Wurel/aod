...
void get_mot(char *mot, int M, int fichier, size_t taille_f){
  // char *mot = calloc(M,sizeof(char));
  PREC_TAMPON = TAMPON;
...
char * completion(char * ligne, int nombre_characteres, int M, int taille_dernier_mot){
  char *nv_ligne = calloc(2*M,sizeof(char));
  char *tampon = calloc(1, sizeof(char));
  int i = 0;
...
void justification(char **tableau_mots, int *tab_argmin, int M, FILE *sortie){
  char *ligne = calloc(M,sizeof(char));
  int debut = 0;
  int temp;
  ...
  while (debut != NB_MOT-1 && debut != fin) {
    ...
    char *nv_ligne = calloc(M,sizeof(char));
...
int main(int argc, char const *argv[]) {
  ...
  char *sort = malloc(sizeof(char)*(strlen(argv[2])+5));
  strncat(sort, argv[2], strlen(argv[2]) - 3);
  // printf("%s\n", sort);
  sortie = fopen(strcat(sort,".out"), "w+");
  int M = atoi(argv[1]);
  while (STATUS != 0) {
    NB_MOT =0;
    ...
    STATUS_PARAGRAPHE = 1;
    struct paragraphe *tete = calloc(1, sizeof(struct paragraphe));
    char *mot = calloc(M,sizeof(char));
    char *ligne = calloc(M,sizeof(char));
    int nombre_characteres = 0;
    struct paragraphe *para = calloc(1,sizeof(struct paragraphe));
    para = tete;
    // para -> place = -1;
    while (STATUS_PARAGRAPHE != 0) {
        struct paragraphe *nv_para = calloc(1,sizeof(struct paragraphe));
        para -> suivant = nv_para;
        nv_para -> mot = calloc(1,sizeof(char) * M);
        // nv_para -> place = para -> place + 1 ;
        get_mot(nv_para -> mot, M, fichier, taille_f);
        para = nv_para;
      }
      ...
      long *tab_coutmin = calloc(NB_MOT,sizeof(long));
      int *tab_argmin = calloc(NB_MOT,sizeof(int));
